...
- [x] `owners[]` includes owners of both the SP and (when present) the Application,
      each tagged with `owner` and `ownerType`.
- [x] `ownerType` distinguishes `user`, `servicePrincipal`, and `group` so non-human
      owners (privilege chains) are visible.
- [x] An SP with no Application object reports only SP-side owners without error.
- [x] Owner reads page fully.
- [x] A failure on an owners call records an SP Gap and the run continues.
